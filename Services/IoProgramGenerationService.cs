...
public class IoProgramGenerationService : IIoProgramGenerationService
{
        ...
        string projectRoot)
    {
        ...
        {
            await Compat.WriteAllTextAsync(artifact.OutputPath, artifact.Content, Encoding.Unicode);
        }
    ...
    private static string BuildDeclarationName(IoSignal signal)
    {
        var basis = string.IsNullOrWhiteSpace(signal.Comment)
            ? NormalizeAddress(signal.Address)
            : $"{NormalizeAddress(signal.Address)}_{signal.Comment}";
        ...
        if (builder.Length == 0)
        {
            builder.Append(NormalizeAddress(signal.Address));
        }
